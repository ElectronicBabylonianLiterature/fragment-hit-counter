...
The hit counter script extracts fragments hits from a catalogue. For each fragment in the catalogue the number of unique documents not starting with `BC` or `CAD` is calculated and saved to the database under `hits` property.
