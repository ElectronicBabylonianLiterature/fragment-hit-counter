...
function readFiles () {
  readdirSync = jest.spyOn(fs, 'readdirSync').mockReturnValue(_.keys(files).concat('invalid.xlsx'))
  readFileSync = jest.spyOn(fs, 'readFileSync').mockImplementation(fileName => windows1252.encode(files[fileName.slice(directory.length + 1)]))
  extractDocuments.mockImplementation(catalog => references[catalog])
  return extractFromDir(directory)
}
