...
module.exports = function mapName (fileName) {
  const id = [
    new IdMapper(
      /^(?:(?<collection>Sm|DT|Rm|Rm 2|Rm II|BM|CBS|N|UM|LB) )?(?<number>\d+)\.csv$/,
      createCollectionName
    ),
    new IdMapper(
      /^(?<collection>CBS|N|UM|Ni|Si|HS|VAT|ANT|AOS-1|DavidAstr|EAH|GCBC|Lager|MLC|NCBS|NCBT|Newell|Ott-Seal|Ott-Tablet|RBC|YBC|LB) (?<number>.+)\.csv$/,
